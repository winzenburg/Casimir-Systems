...
human-readable briefing. Never submit anything or contact anyone on Casimir's
behalf.
...
- **Flag persistent source outages by duration, not just as a one-off.**
  `reports/source-health.json` tracks how many consecutive days each source
  has been failing; `scan_opportunities.py`'s `annotate_source_health()`
  appends that to the status line (e.g. `[down since 2026-07-03 (5d)]`). If a
  source has been down for more than a few days, say so explicitly rather
  than repeating "rerun the scan later" as if it were a one-off blip.
...
- `reports/`: dated briefings plus `seen.json` (the new-since-last-scan
  state), `source-health.json` (consecutive-failure tracking per source),
  and `latest-summary.json` (machine-readable counts for CI). All committed
  to git so scheduled runs archive their output; only raw `--json` dumps
  stay local.
